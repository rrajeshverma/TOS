...
def test_stream_can_subscribe_symbol():
    ...
    assert "NIFTY" in stream.subscriptions()
...
def test_stream_can_unsubscribe_symbol():
    ...
    assert "NIFTY" not in stream.subscriptions()
...
def test_stream_publishes_event():
    ...
    assert stream.latest_tick("NIFTY") == event.tick
...
def test_stream_returns_none_for_unknown_symbol():
    ...
    assert stream.latest_tick("BANKNIFTY") is None
...
def test_stream_keeps_latest_tick():
    ...
    assert stream.latest_tick("NIFTY").price == 24500.50
