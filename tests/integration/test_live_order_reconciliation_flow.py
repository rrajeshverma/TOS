...
def test_order_registration():
    ...
    service.register_order("ORDER001")

    assert service.get_status("ORDER001") == OrderState.PENDING


def test_order_status_updates_after_broker_response():
    service = create_service()

    service.register_order("ORDER002")
    ...
    assert service.get_status("ORDER002") == OrderState.FILLED


def test_unknown_order_can_be_reconciled():
    service = create_service()

    service.register_order("ORDER003")
    ...
    assert result == OrderState.UNKNOWN
...
def test_complete_order_recovery_flow():
    ...
    service.register_order("ORDER004")
