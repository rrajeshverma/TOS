...
def test_multiple_metrics_are_tracked():
    ...
    metrics.increment("successful_orders")
    ...
    assert snapshot["successful_orders"] == 1
...
def test_unknown_metric_is_rejected():
    ...
    try:
        ...
        assert str(exc) == "Unknown metric: unknown"
...
def test_reset_clears_metrics():
    ...
    metrics.increment("failed_orders")

    metrics.reset()

    assert metrics.get("orders") == 0

    assert metrics.get("failed_orders") == 0
