...
def main() -> int:
    ...
    print(
        "Broker        :",
        "PAPER CONNECTED" if broker.is_connected() else "NOT CONNECTED",
    )
    ...
    print(
        "Market        :",
        "OPEN" if trading_session.is_market_open(current_time) else "CLOSED",
    )

    print(
        "Entry Window  :",
        "ACTIVE" if trading_session.is_entry_allowed(current_time) else "INACTIVE",
    )
